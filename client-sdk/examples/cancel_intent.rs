...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user_address = std::env::var("USER_ADDRESS")
        .map_err(|_| "USER_ADDRESS is required")?;
    let signature_r = std::env::var("USER_SIGNATURE_R")
        .map_err(|_| "USER_SIGNATURE_R is required")?;
    let signature_s = std::env::var("USER_SIGNATURE_S")
        .map_err(|_| "USER_SIGNATURE_S is required")?;
    ...
    client
        .cancel_intent(&intent_id, &user_address, vec![signature_r, signature_s])
        .await?;

    println!("✅ Intent {} canceled", intent_id);
